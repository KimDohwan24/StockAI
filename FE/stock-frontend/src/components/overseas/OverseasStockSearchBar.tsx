...
export default function OverseasStockSearchBar({ inline }: { inline?: boolean } = {}) {
  ...
  const { data: results, isLoading } = useSWR<
    { items: OverseasStockCatalogItem[] }
  >(
    debouncedQuery.trim().length >= 1 ? `overseas-search-${debouncedQuery}` : null,
    async () => {
      const res = await searchOverseasStocks({ query: debouncedQuery, size: 8 });
      return { items: res.content };
    },
    { dedupingInterval: 500 }
  );

  const showDropdown = isFocused && (results?.items?.length ?? 0) > 0;

  const handleSelect = useCallback(
    (ticker: string, exchangeCode: string) => {
      setIsFocused(false);
      setQuery('');
      setDebouncedQuery('');
      router.push(`/overseas-stocks/${ticker}?exchange=${exchangeCode}`);
    },
    [router]
  );
    ...
    return (
      <div ref={containerRef} className="relative flex-1 min-w-0 flex items-center gap-2">
        ...
        <input
          ...
          placeholder="해외 종목 검색 (예: AAPL, Tesla)"
        ...
        {showDropdown && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-hairline-soft rounded-xl shadow-lg z-50 max-h-60 overflow-auto">
            ...
            {results?.items?.map((item) => (
              <button
                key={`${item.ticker}-${item.exchangeCode}`}
                type="button"
                onClick={() => handleSelect(item.ticker, item.exchangeCode)}
                className="w-full text-left px-4 py-2.5 hover:bg-surface-soft transition-colors text-sm"
              >
                <span className="font-bold">{item.name}</span>
                <span className="text-steel ml-2">({item.ticker})</span>
                <span className="text-stone ml-2 text-xs">{item.exchangeCode}</span>
              </button>
            ))}
          </div>
        )}
